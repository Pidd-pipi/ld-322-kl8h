...
import { Badge, Button, Card, Col, Empty, Row, Select, Space, Spin, Typography, message } from 'antd';
...
import { isOnline } from '../utils/sensorStatus';
...
export default function DashboardPage() {
  ...
  const refresh = useCallback(async (id = selected) => {
    if (!id) return;
    try {
      const [info, latest, deviceRows, alertRows] = await Promise.all([
        getGreenhouse(id), getLatest(id), getDevices(id), getAlerts(id),
      ]);
      setDetail(info);
      setReadings(latest);
  ...
  const offlineCount = sensors.filter((sensor) => !isOnline(sensor, now)).length;
  const onlineCount = sensors.length - offlineCount;
    ...
    <Space direction="vertical" size={20} style={{ width: '100%' }}>
      <div className="page-heading">
        <div>
          ...
          <Text type="secondary">每 30 秒自动刷新 · WebSocket 实时报警推送 · 5 分钟未上报自动标记离线</Text>
        </div>
        <Space>
          <Select
            aria-label="选择温室"
      ...
      <Card
        ...
        extra={
          <Space size={24} wrap>
            <Text>{detail?.location} · {detail?.area}㎡</Text>
            <Space size={16}>
              <Badge status="success" text={<Text>在线 <strong>{onlineCount}</strong></Text>} />
              <Badge
                status={offlineCount > 0 ? 'error' : 'default'}
                text={<Text type={offlineCount > 0 ? 'danger' : undefined}>离线 <strong>{offlineCount}</strong> / {sensors.length}</Text>}
              />
            </Space>
          </Space>
        }
      >
        <MetricCards sensors={sensors} readings={readings} now={now} />
      </Card>

      <Row gutter={[16, 16]}>
        <Col xs={24} lg={15}>
          <Card title="24 小时环境趋势" extra={<Button size="small" onClick={() => void refresh()}>立即刷新</Button>}>
        ...
        <Col xs={24}>
          <SensorStatusTable sensors={sensors} readings={readings} now={now} />
        </Col>
        <Col xs={24}>
